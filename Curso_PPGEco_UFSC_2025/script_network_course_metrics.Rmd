...
author: "Carine Emer"
date: "11/10/2018 - updated 27/10/2025"
...
# Z-score and p-value
z_score <- (nested_ants - mean_null) / sd_null
p_value <- sum(null1 >= nested_ants) / length(null1)  # dividing by 100
...
abline(v = nested_ants, col = "red", lwd = 2)
...
is.numeric(ants_b)
str(ants_b)
ants_b_matrix <- as.matrix(ants_b)
mode(ants_b_matrix) <- "numeric"
btw_ants<-specieslevel(ants_b_matrix,index = "betweenness",level = "higher")
btw_plants<-specieslevel(ants_b_matrix,index = "betweenness",level = "lower")

### closenness
clo_ants<-specieslevel(ants_b_matrix,index = "closeness",level = "higher")
clo_plants<-specieslevel(ants_b_matrix,index = "closeness",level = "lower")
